...
void Foam::janaf9Thermo<EquationOfState>::checkInputData() const
{
    if (Tlow_ >= Thigh_)
    {
    ...
    {
        FatalErrorInFunction
            << "Tcommon2(" << Tcommon_ << ") => Thigh(" << Tlow_ << ')'
            << exit(FatalError);
    }
    
    if (Tcommon2_ < Tcommon_)
    {
        FatalErrorInFunction
            << "Tcommon2(" << Tcommon_ << ") <= Tcommon(" << Tlow_ << ')'
            << exit(FatalError);
    }
...
:
    ...
    Tcommon2_(dict.subDict("thermodynamics").get<scalar>("Tcommon2")),
    highCpCoeffs_(dict.subDict("thermodynamics").lookup("highCpCoeffs")),
    middleCpCoeffs_(dict.subDict("thermodynamics").lookup("middleCpCoeffs")),
    lowCpCoeffs_(dict.subDict("thermodynamics").lookup("lowCpCoeffs"))
{
    ...
    for (label coefLabel=0; coefLabel<nCoeffs_; coefLabel++)
    {
        ...
        middleCpCoeffs_[coefLabel] *= this->R();
...
void Foam::janaf9Thermo<EquationOfState>::write(Ostream& os) const
{
    ...
    // Entries in dictionary format
    {
        ...
        os.writeEntry("Tcommon2", Tcommon2_);
        os.writeEntry("highCpCoeffs", highCpCoeffs);
        os.writeEntry("middleCpCoeffs", middleCpCoeffs);
